...
import { createAdvertisement, directFileUpload } from '../../api/doctor/doctorApi';
...
const DoctorAdvertisementCreate = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    video: add?.video || '',
    newVideo: '',
    location: add?.location || { type: "Point" as const, coordinates: [NaN, NaN] as [number, number], text: '' },
    tags: add?.tags || [],
  });
  ...
  useEffect(() => {
    if (add) {
      setFormData({
        ...
        video: add.videoUrl || '',
        newVideo: '',
        location: add.location || { type: "Point", coordinates: [NaN, NaN], text: '' },
        tags: add.tags || [],
      });
  ...
  const validateForm = useCallback(() => {
    ...
    else if (formData.title.length < 3) newErrors.title = 'Title must be at least 3 characters long';
    ...
    if (!formData.location.text.trim()) newErrors.location = 'Location is required';
    ...
    else if (formData.tags.length > 10) newErrors.tags = 'Maximum 10 tags allowed';
    if (videoFile) {
      const maxSize = 50 * 1024 * 1024; // 50MB
      if (videoFile.size > maxSize) newErrors.video = 'Video file size exceeds 50MB';
      if (!videoFile.type.startsWith('video/')) newErrors.video = 'Please select a valid video file';
    }
    setErrors(newErrors);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = typeof err === 'object' && err !== null && 'message' in err
        ? (err as { message?: string }).message || 'Failed to save advertisement. Please try again.'
  ...
  const VideoUploadSection = () => (
    <div>
      <label className="flex items-center gap-2 text-xs sm:text-sm md:text-base font-semibold text-gray-800 mb-2 sm:mb-3">
        ...
        <span className="text-xs text-gray-500 font-normal">(Max 50MB, 16:9 preferred)</span>
      </label>
      <div className="relative">
        ...
        {!(formData.newVideo || formData.video) ? (
          ...
          >
            ...
            <span className="text-xs sm:text-sm md:text-base font-medium text-gray-600">Click to upload video</span>
            <span className="text-xs text-gray-400 mt-1 sm:mt-2">MP4, MOV, AVI up to 50MB</span>
        ...
        {videoFile && (
          <div className="mt-2 text-xs sm:text-sm text-gray-600 flex items-center gap-2 flex-wrap">
            <FaVideo className="text-blue-600" />
            <span className="truncate max-w-[150px] sm:max-w-[200px] md:max-w-[300px]" title={videoFile.name}>
              {videoFile.name}
            </span>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
            <div className="w-full sm:w-auto">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-600 mt-1">
                {add ? 'Update your existing ad video' : 'Create a video ad to promote your services'}
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
              ...
              <button
                ...
                disabled={loading}
              >
                <FaSave className="text-xs sm:text-sm" /> {loading ? 'Saving...' : 'Save'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8">
          ...
          <div>
            <label className="flex items-center gap-2 text-xs sm:text-sm md:text-base font-semibold text-gray-800 mb-2 sm:mb-3">
              ...
              <span className="text-xs text-gray-500 font-normal">({formData.tags.length}/10)</span>
            </label>
