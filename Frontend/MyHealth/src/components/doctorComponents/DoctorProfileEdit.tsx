...
const EditDoctorProfileModal = ({
    ...
    switch (name) {
      case "fullName":
        if (!value.trim()) error = "Full name is required";
          ...
          error =
            "Full name can only contain letters, spaces, periods, or hyphens";
        break;
      case "phone":
      ...
      case "graduation":
        if (!value.trim()) error = "Graduation is required";
          ...
          error = "Graduation can only contain letters, spaces, or commas";
        break;
      case "category":
  ...
  const validateForm = (): boolean => {
    ...
    if (
      !formData.location ||
      (formData.location && !formData.location.text) ||
    ...
    if (!formData.graduation || formData.graduation.trim().length === 0) {
      ...
      newErrors.graduation =
        "Graduation can only contain letters, spaces, or commas";
    }
    ...
    if (!formData.category || formData.category.trim().length === 0) {
      ...
      newErrors.category =
        "Category can only contain letters, spaces, or hyphens";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      const submitData: doctorProfileUpdate = {
        ...
        experience:
          formData.experience === null ? undefined : formData.experience,
      };
