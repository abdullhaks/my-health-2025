  ...
  implements IUserPrescriptionCtrl
{
  constructor(
    ...
    private _prescriptionService: IUserPrescriptionService
  ) {}
  ...
  async getLatestPrescription(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (userId) {
        const response = await this._prescriptionService.getLatestPrescription(
          userId.toString()
        );
        // if(!response){
        //     res.status(HttpStatusCode.BAD_REQUEST).json({message:"bad request"});
  ...
  async getLatestDoctorPrescription(
    ...
    try {
      ...
      if (userId && doctorId) {
        const response =
          await this._prescriptionService.getLatestDoctorPrescription(
            userId.toString(),
            doctorId?.toString()
          );
        // if(!response){
