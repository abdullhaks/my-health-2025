...
@injectable()
export default class DoctorPrescriptionController
  implements IDoctorPrescriptionCtrl
{
  constructor(
    @inject("IDoctorPrescriptionService")
    private _doctorPrescriptionService: IDoctorPrescriptionService
  ) {}
  ...
  async getPrescriptions(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (userId) {
        const response = await this._doctorPrescriptionService.getPrescriptions(
          userId.toString()
        );
        if (!response) {
          res
