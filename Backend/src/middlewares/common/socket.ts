...
export const setupSocket = (io: Server, container: Container) => {
  ...
  const prescriptionService = container.get<IUserPrescriptionService>(
    "IUserPrescriptionService"
  );
  const appointmentsRepository = container.get<IAppointmentsRepository>(
  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    socket.on(
      ...
      async (msg: {
        ...
        if (msg.senderId !== userId) {
          console.warn(
            `Unauthorized message attempt by ${userId} for senderId ${msg.senderId}`
          );
    ...
    socket.on("typing", ({ conversationId }: { conversationId: string }) => {
      socket
        .to(conversationId)
        .emit("typing", { userId, role, conversationId });
    });
    ...
    socket.on("joinVideoCall", async (appointmentId: string) => {
      ...
      try {
        ...
        if (appointment.appointmentStatus !== "booked") {
          console.error(
            `Appointment ${appointmentId} is not in booked status.`
          );
          return socket.emit("error", {
            message: `Appointment is ${appointment.appointmentStatus}.`,
          });
        }
        ...
        if (
          ...
          console.error(
            `Unauthorized access attempt to appointment ${appointmentId} by user ${userId}.`
          );
          return socket.emit("error", {
        ...
        if (now < fiveMinutesBeforeStart) {
          socket.emit("error", {
            message:
              "You can only join the meeting within 5 minutes before the start time.",
          });
          return;
        }
        if (now > endTime) {
        ...
        console.error(
          `Error joining video call for ${userId} in ${appointmentId}:`,
          err
        );
        ...
        if (role === "doctor") {
          try {
            if (prescriptionSubmited) {
              const updating = await appointmentsRepository.update(
                appointmentId,
                {
            ...
            console.error(
              `Error processing doctor leave for ${appointmentId}:`,
              err
            );
            socket.emit("error", {
